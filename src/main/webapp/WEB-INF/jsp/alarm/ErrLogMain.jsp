...
<script>
	...
	function fnInitCtrl(){
		...
		$("#btnExcel").click(function(){
			...
			argoJsonSearchList('errCode', 'getErrLogList', 's_', {"iSPageNo":100000000, "iEPageNo":100000000}, function (data, textStatus, jqXHR){
				try {
					if (data.isOk()) {
	...
	function fnInitGrid(){
		...
		$('#gridList').w2grid({ 
	        ...
	        columns: [  
	            	 	...
	            	 	,{ field: 'errCode', 		caption: '장애코드', 		size: '7%', 	sortable: true, attr: 'align=center' }
	            		,{ field: 'errTypeName', 	caption: '장애구분', 		size: '7%', 	sortable: true, attr: 'align=center' }
	            		,{ field: 'errGradeName', 	caption: '장애등급', 		size: '9%', 	sortable: true, attr: 'align=center' }
	            		,{ field: 'systemName', 	caption: '시스템명', 		size: '13%', 	sortable: true, attr: 'align=center' }
	            		...
	            		,{ field: 'errIndex', 		caption: 'errIndex', 	size: '0%', 	sortable: true, attr: 'align=center' }
		...
		w2ui['grid'].hideColumn('recid', 'errKey', 'errIndex', 'errType', 'errGrade', 'systemId', 'processId');
	}
	...
	function fnSearchListCnt(){
		...
		argoJsonSearchOne('errCode', 'getErrLogCount', 's_', {}, function (data, textStatus, jqXHR){
			try{
				if(data.isOk()){
					if(data.getRows() != ""){
	...
	function fnSearchList(startRow, endRow){
		...
		argoJsonSearchList('errCode', 'getErrLogList', 's_', {"iSPageNo":startRow, "iEPageNo":endRow}, function (data, textStatus, jqXHR){
			try{
				if (data.isOk()) {
					w2ui.grid.clear();
...
<body>
	<div class="sub_wrap">
        <div class="location"><span class="location_home">HOME</span><span class="step">장애관리</span><strong class="step">장애이력조회</strong></div>
