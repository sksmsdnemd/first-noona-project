...
<script>
    ...
    function displayRecPlayBtn() {
        ...
        //if ($("#chkListeningYn").is(':checked') || authRank < 4)  $('#btnStt').show(); else $('#btnStt').hide();
    }
    function displayRecDownloadBtn() {
    	console.log("displayRecDownloadBtn() : " + authRank + $("#chkDownloadYn").is(':checked'));
...
// 					$("#s_FindReasonText").val('');		//조회사유
	...
	function fnRecSearchReasonAddPop(){
		...
		var pChkListeningYn = argoNullConvert(argoGetValue("chkListeningYn"));
		...
		setTimeout(function() {
	        ...
	        argoJsonSearchList('recSearch', 'getRecSearchList', '_', callbackAfterSearchCondition, function(data, textStatus, jqXHR) {
	            try {
                    ...
                    if(dataArray.length > 0 && $("#chkListeningYn").prop("checked") == true){
                    	displayRecPlayBtn();
                    }else{
                    	$("#btnPlay").hide();
                    }
                    
                    if(dataArray.length > 0 && $("#chkDownloadYn").prop("checked") == true){
                        displayRecDownloadBtn();
                    }else{
                    	$("#btnWavConv").hide();
                    }
			...
			<div class="search_area row5" id="searchPanel">
				...
				<div class="row" style="width: 1350px;">
					<ul class="search_terms">
						...
						<li id="is_callexcept_node"><strong class="title" ml20>녹취권한</strong>
