...
<script>
	...
	function fnInitCtrl(){		
		...
		$("#btnExcel").click(function(){
			...
			argoJsonSearchList('recSearch', 'getRecChartUserList', 's_', {userId:userId, controlAuth:controlAuth, grantId:grantId}, function (data, textStatus, jqXHR){
				try {
					if (data.isOk()) {
						$.each(data.getRows(), function( index, row ) {
							...
							gObject = {   "순번" 		: index + 1
										...
										, "통화수" 	: row.callCnt
										, "평균통화수" 	: row.callAvg
										, "총통화시간" 	: fnSecondsConv(row.timeSum)
										, "평균시간" 	: fnSecondsConv(row.timeAvg)
									};
	...
	function fnInitGrid(){
		...
		$('#gridList').w2grid({ 
	        ...
	        columns: [  
			           	 { field: 'userName', 	caption: '상담사명', 	size: '80px', sortable: true, attr: 'align=center' }
		            	,{ field: 'userId', 	caption: '상담사ID', 	size: '80px', sortable: true, attr: 'align=center' }
			           	,{ field: 'callCnt', 	caption: '통화수', 	size: '60px', sortable: true, attr: 'align=center' }
			           	,{ field: 'callAvg', 	caption: '평균통화수', 	size: '75px', sortable: true, attr: 'align=center' }
			           	,{ field: 'timeSum', 	caption: '총통화시간', 	size: '85px', sortable: true, attr: 'align=center' }
			           	,{ field: 'timeAvg', 	caption: '평균시간', 	size: '85px', sortable: true, attr: 'align=center' }
	        ],
	        records: dataArray
	    });	 
	}
					...
					if (data.getRows() != ""){ 
						$.each(data.getRows(), function( index, row ) {
							...
							gridObject = {  
											...
											, "callCnt" 	: row.callCnt
											, "callAvg" 	: row.callAvg
											, "timeSum" 	: fnSecondsConv(row.timeSum)
											, "timeAvg" 	: fnSecondsConv(row.timeAvg)
										};
					    	...
					    	chartData.push(
					    		{
									...
									,"callCnt"	: row.callCnt
									,"callAvg"	: row.callAvg
									,"timeSum"	: Math.ceil(row.timeSum/60)
									,"timeAvg"	: row.timeAvg
					    		}
					    	);
						});
						...
						var char = AmCharts.makeChart("div_Chart",
							{
							    ...
							    "fontSize": 12,
								"colors": [
									"#5571c6", "#db9d61", "#55c6c2", "#CC0000", "#0D8ECF", "#2A0CD0", "#CD0D74", "#00CC00",	"#0000CC", "#DDDDDD",
...
<body>
	<div class="sub_wrap">
        <div class="location">
        	<span class="location_home">HOME</span><span class="step">통화내역관리</span><span class="step">통화내역집계</span><strong class="step">상담사별 통계</strong>
        </div>
	            ...
	            <div>
	            	<ul class="search_terms">
	            		<li style="width:520px">
	            			<div id="gridList" style="height:420px;"></div>
	            		</li>
	            		<li style="width:10px">&nbsp;</li>
	            		<li style="width:570px">	
	            			<div class="cont_box">
	            				<div id="div_Chart" style="height:400px;"></div>
	            			</div> 
	            		</li>
	            	</ul>
	            </div>
