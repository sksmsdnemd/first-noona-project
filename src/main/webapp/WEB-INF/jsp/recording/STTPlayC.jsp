...
%>
	...
	<script type="text/javascript" src="<c:url value="/scripts/player/player_api_Chrome.js"/>"></script>
<%
...
// 	String strLogURL 	= "http://" + strIP + ":" + strPort + "/rec_log?call_id="+strCallID+"&tenant_id=VLC_SSO_TENANT_ID&worker_id="+strManagerID+"&realtime_flag=0&reason=RTITLE&reason_text=RTEXT";
	...
	String strLogURL 	= protocol + strIP + ":" + strPort + "/rec_log?call_id="+strCallID+"&tenant_id=VLC_SSO_TENANT_ID&worker_id="+strManagerID+"&realtime_flag=3&reason=RTITLE&reason_text=RTEXT";
%>
	<script type="text/javascript">
					...
					if(data.isOk()){
						if(data.getRows() != ""){
							...
							if(isUseRecReason == "1"){
								...
								var logWorkId =  "<%=strManagerID%>";
								var logDnNo =  "<%=strDnno%>";
								var logCallId = "<%=strCallID%>";
								var logTenantId = "<%=strTenant%>";
								var strLoginIp = "<%=strLoginIp%>";
								var logRealtimeFlag = "3";
								
								argoJsonUpdate("recInfo", "setAppRecLogRealInsert", "", {"tenantId":logTenantId,"workerId":logWorkId, "dnNo":logDnNo, "listeningKey":logListeningKey 
									, "callId" : logCallId ,"realtimeFlag":logRealtimeFlag ,"loginIp":strLoginIp });
		...
		function getWindowPopupOpen(){
			var index = {};
			index.workId = "<%=strManagerID%>";
			index.dnNo = "<%=strDnno%>";
			index.callId = "<%=strCallID%>";
			index.tenantId = "<%=strTenant%>";
			index.loginIp = "<%=strLoginIp%>";
...
// 				};
		...
		function playAudio() {
			player.play_pause(function(isRunning) {
				...
				var strUrl 		= "<%=strURL%>";
					//w2ui['grid'].getCellValue(arrChecked[0], 1);
...
<body>
	<div id="playerCover" style="height: 135px;">
