...
<script>
	...
	function dnExcelSampleDownload(){
		var form = $('<form>');
		var actionUrl = gGlobal.ROOT_PATH + "/RecSearch/RecSearchCustInfoUpdateNewF.do?"+param+"&excelImSvcName=recSearchNew"
											+"&excelImMethodName=getRecSearchListExcelImportNew&disInsColNum=8&fileName=UserInfoUpdate.xlsx";
	...
	function fnExcelInsert(){
		...
		formData.append("insColNum",12);
		formData.append("insSeedNum",11);
		$.ajax({
			url : gGlobal.ROOT_PATH+"/RecSearch/CustInfoExcelInsertNewF.do",
			type : "POST",
			data:formData,  
