...
%>
	...
	<script type="text/javascript" src="<c:url value="/scripts/player/player_O.js"/>"></script>
...
// 			String strLogURL 		= "http://" + strIP + ":" + strPort + "/rec_log?call_id="+strCallID+"&tenant_id=VLC_SSO_TENANT_ID&worker_id="+strManagerID+"&realtime_flag=0";
...
%>
    ...
    /*
		...
		playPause.on('click', function(){ 
			changePlayCon();
			if(audio.paused){
				// ie의 경우 pause후 play를 할 경우 기존의 playbackRate가 1로 초기화 되는 현상 발생하여 play 후 기존 playbackRate를 다시 셋팅함.
				var pastPlaybackRate = audio.playbackRate;
				audio.play();
				audio.playbackRate = pastPlaybackRate;
			}else{
				audio.pause();
			}
		});
...
// 		}
	...
	function checkRecLog(recid){
		// recRog에 없으면
		if (recLogFlagArray.indexOf(String(recid)) == -1 
			|| recLogFlagArray.indexOf(String(recid)) == "-1"){
			confirmRecLog(String(recid));
		}else{
			playAudio(recid);
		}
...
<body>
	<div id="playerCover">
		...
		<div id="gridList" style="margin:1.8% 3% 0 3%;height: 170px;background-color: #ffffff;"></div>
	</div><!--playerCover-->
</body>
</html>
