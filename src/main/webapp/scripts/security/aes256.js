...
var key = "bridgetec_argo_security123456789";
function AES_Encode(plain_text)
{
	var size = 256;
	if(top.gConfig.EN_IN_TYPE==0){
		size = 128;
	}
	var rtStr = "";
	try{
		GibberishAES.size(size);
		rtStr = GibberishAES.aesEncrypt(plain_text, key);
	} catch (e) {
		rtStr = plain_text;
	}
...
function AES_Decode(base64_text)
{
	...
	try{
		GibberishAES.size(size);	
		rtStr = GibberishAES.aesDecrypt(base64_text, key);		
	} catch (e) {
		rtStr = base64_text;
	}
	return rtStr;
