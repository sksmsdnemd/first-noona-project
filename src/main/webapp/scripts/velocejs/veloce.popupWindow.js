...
(function($){
	...
	$.fn.openVeloceWindow = function( options ) {
...
function VeloceWindow_view( selector, url, title, width, height, pid, ptype, multi, closeable, target, callback, method )
{	
...
VeloceWindow_view.prototype.init = function()
{
...
var id_num = 0;
var div_num = 0;
VeloceWindow_view.prototype.make_window = function()
{
    ...
    html +=         	'<iframe id="' + this.target + '" name="' + this.target + '" src="' + this.url + '" data-pid="'+ this.pid +'" data-ptype="'+ this.ptype +'" ></iframe>'
    html +=         '</div></div></div></div>'
	$("body").append( html );
	setTimeout(function(){ 	$(".pop_bgLayer").css("opacity", "0.4");},10);
}

VeloceWindow_view.prototype.multi_window = function()
{
	div_num++;	
	var html = "";
...
var win_depth = 100;
VeloceWindow_view.prototype.addEvent = function()
{
	var thisObj = this;
	this.btn_close.on( "click", function()
	{
		...
		if (parent.gPopupOptions && parent.gPopupOptions.grid) {
			parent.gPopupOptions.grid.selectNone();
		}
	});
...
VeloceWindow_view.prototype.window_position = function(time)
{	
	var objThis = this;
	setTimeout(function(){		
...
VeloceWindow_view.prototype.removePop = function(_this, _length)
{
...
//타이틀, url, 창 넓이, 창 높이
function velocePopupWindow( _title, _url, _width, _height,_multi, _target, _callback, _method)
{
    ...
    if(window.frameElement){
	    ...
	    if(isPopup) { // 팝업에서 팝업을 호출 할 경우 
		    ...
		    openWindow = parent.$("body").openVeloceWindow({
							title:_title,
							url:_url,
							width:_width,
							height:_height,
	    	...
	    	parent.gPopupOptions = gPopupOptions ;
	    	openWindow = $("body").openVeloceWindow({
		    			title:_title,
		    			url:_url,
		    			width:_width,
		    			height:_height,
...
//팝업 닫기
function velocePopupClose()
{
	$(window.frameElement.parentNode).closest('.pop_layer').remove();
	if(typeof(parent.gPopupOptions) != "undefined"){
		parent.gPopupOptions.grid.selectNone();
	}
}
