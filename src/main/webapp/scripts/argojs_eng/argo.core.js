...
function ArgoService(svcId, svcName, methodName, svcType, dbType, inType, inPrefixName, outType, outName, addParam) {
    ...
    this.inType       = inType;
    this.inPrefixName = inPrefixName;
...
function argoJson(sJsonString) {
    ...
    */
    ...
    if(jsonResult && jsonResult[gGlobal.RESULT_CODE]) {
        ...
        //서비스처리결과메시지
        jsonResult.getMessage = function() {
            return "There is an error communicating with the server. Please try again later.";
        };
        //서비스처리결과세부코드
        jsonResult.getSubCode = function() {
            return "";
    ...
    if (fnError == null) fnError = function(jqXHR, textStatus, errorThrown) {
        ...
        if(isErrorAlert == false) {
           ...
           // alert("서버와의 통신에 오류가 있습니다. 잠시 후 다시 시도하여 주세요."+textStatus, function() {
               argoAlert('warning',"There is an error communicating with the server. <br>Please try again later."+"["+textStatus +"]", function() {
                isErrorAlert = false;
            });
        }
    };
...
function argoAjaxUpload(oForm, fnCallback){
    ...
    var options = { 
        ...
        error: function(){
            ...
            if(isErrorAlert == false) {
                isErrorAlert = true;
                alert("There is an error communicating with the server. Please try again later.", function() {
                    isErrorAlert = false;
                });
            }
        }
		 ...
		 try{	        
		        ...
		        var options = { 
		            ...
		            error: function(){
		                ...
		                if(isErrorAlert == false) {
		                    isErrorAlert = true;
		                    alert("There is an error communicating with the serve. Please try again later.", function() {
		                        isErrorAlert = false;
		                    });
		                }
		            }
...
function argoResultDialog(code, msg, subCode, subMsg) {
	try{
		alert("Result Code : " + code + "\n" + msg+"\n Detail Result Code : " + subCode + "\n" + subMsg);
	}catch(e){
		alert(e);
	}
}
    ...
    function load(){
       ...
       if($('#'+$that.publicKeyExponent).length < 1)
        ...
        if(result.isOk()){
            ...
            alert('Failed to get RSA public key. Please check WAS.');
        }
    }
