...
public class ByteUtil
{
    ...
    public static final long getlong(byte src[], int offset)
    {
        return 
            ...
            getint(src, offset + 4) & 0xffffffffL;
    }
	
	public static final long getlong(byte src[]){
		return
		    (((long)(src[0] & 0xff) << 24) & 0xffffffff ) | 
		    ((src[1] & 0xff) << 16) | 
		    ((src[2] & 0xff) << 8) | 
		    (src[3] & 0xff) ;
	}
    ...
    public static byte[] fillInBuff(byte[] buf,int start,int offset,String msg)
	{
		if(msg == null) msg="";
    ...
    public static byte[] fillInBuffSpace(byte[] buf,int start,int offset,String msg)
	{
		if(msg == null) msg="";
		byte[] tmp=msg.trim().getBytes();
		for(int i=start;i<offset+start;i++)
